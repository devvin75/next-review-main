...
// transforming the array of obejects into an array of strings
...
export async function getReview(slug) {
  ...
  // CHECK FOR ERRORS -- If the slug we requested does not match any reviews in the CMS
  // then the data array will be empty
  if(data.length === 0){
    return null; //we use null to indicate that the review was not found
  }
...
export async function getMoreReviews(pageSize) {
  // const url = `${CMS_URL}/api/reviews?`
  //   + qs.stringify({
...
// GET THE SLUGS OF THE REVIEWS IN THE CMS
export async function getSlugs() {
  ...
  // RETURN AN ARRAY WITH ALL THE SLUGS
